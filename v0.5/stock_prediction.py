...
arima_data = data[PRICE_VALUE].values
size = int(len(arima_data) * 0.8)
...
predictions_arima = model_arima_fit.forecast(steps=len(test_arima))[0]

ensemble_predictions = (predictions_arima + predicted_prices.squeeze()) / 2
...
plt.plot(test_arima, color="black", label=f"Actual {COMPANY} Price")
plt.plot(predictions_arima, color="red", label="ARIMA Predictions")
plt.plot(predicted_prices, color="blue", label="LSTM Predictions")
plt.plot(ensemble_predictions, color="green", label="Ensemble Predictions")
plt.title(f"{COMPANY} Share Price Forecasting")
plt.xlabel("Time")
plt.ylabel(f"{COMPANY} Share Price")
plt.legend()
plt.show()
...
rmse_lstm = sqrt(mean_squared_error(actual_prices, predicted_prices))
rmse_ensemble = sqrt(mean_squared_error(test_arima, ensemble_predictions))

print(f"RMSE for ARIMA: {rmse_arima}")
print(f"RMSE for LSTM: {rmse_lstm}")
print(f"RMSE for Ensemble: {rmse_ensemble}")
